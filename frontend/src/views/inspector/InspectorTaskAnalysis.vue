<template>
  <div class="inspector-task-analysis">
    <div class="page-header">
      <h3 class="page-title">任务分析</h3>
      <span class="header-subtitle">考试成绩与作业完成情况统一看板</span>
    </div>

    <el-tabs v-model="activeTab" @tab-change="onTabChange">
      <el-tab-pane label="考试成绩分析" name="exams">
        ...
        <div v-if="examLoading" class="class-cards"><div v-for="i in 4" :key="i" class="sk-card"><div class="sk-line w-60" /><div class="sk-line w-80" /><div class="sk-line w-40" /></div></div>
        <div v-else class="class-cards">
          ...
          >
            <div class="cc-header"><span class="cc-grade">{{ c.grade }}</span><span class="cc-name">{{ c.className }}</span></div>
            <div class="cc-metrics">
              ...
              <div class="metric" :class="(c.passRate || 0) >= 60 ? 'good' : 'bad'"><span class="m-val">{{ c.passRate }}%</span><span class="m-lbl">及格率</span></div>
            </div>
            <div class="cc-sub"><span>已提交 {{ c.submittedCount }}</span><span>已批改 {{ c.gradedCount }}</span></div>
            <div class="cc-arrow">查看详情 →</div>
          </div>
          <div v-if="classStats.length === 0 && !examLoading" class="empty-full"><el-empty description="暂无成绩数据" :image-size="80" /></div>
        </div>
      ...
      <el-tab-pane label="作业完成看板" name="homework">
        <div class="filter-bar">
          <el-select
            v-model="hwFilters.grade"
            placeholder="年级"
            clearable
            size="default"
            @change="onHwGradeChange"
          >
            <el-option
              v-for="g in gradeOptions"
              :key="g"
          ...
          <el-button size="default" @click="resetHwFilters"><el-icon><Refresh /></el-icon>重置</el-button>
        </div>
        ...
        >
          ...
          <el-table-column
            ...
            align="center"
          >
            <template #default="{ row }"><span :style="{ color: row.notSubmittedCount > 0 ? 'var(--danger-color)' : 'var(--success-color)' }">{{ row.notSubmittedCount }}</span></template>
          </el-table-column>
          <el-table-column label="提交率" width="80" align="center">
            <template #default="{ row }"><span :style="{ color: row.submitRate >= 80 ? 'var(--success-color)' : row.submitRate >= 50 ? 'var(--warning-color)' : 'var(--danger-color)' }">{{ row.submitRate }}%</span></template>
          </el-table-column>
          <el-table-column
            prop="gradedCount"
            label="已批改"
            ...
            align="center"
          />
          <el-table-column label="批改率" width="75" align="center"><template #default="{ row }">{{ row.gradeRate }}%</template></el-table-column>
          <el-table-column
            prop="avgScore"
            label="均分"
            width="70"
          ...
          <el-table-column label="及格率" width="80" align="center">
            <template #default="{ row }"><span :style="{ color: (row.passRate||0) >= 60 ? 'var(--success-color)' : 'var(--danger-color)' }">{{ row.passRate }}%</span></template>
          </el-table-column>
        </el-table>
        <div v-if="hwList.length === 0 && !hwLoading" class="empty-state"><el-empty description="暂无作业数据" :image-size="80" /></div>
      </el-tab-pane>
    </el-tabs>
  </div>
</template>
...
const detailTitle = computed(() => detailClass.value ? `${detailClass.value.grade} ${detailClass.value.className} — 成绩详情` : '')
...
const openClassDetail = (c) => { detailClass.value = c; detailVisible.value = true }
...
const loadExamData = async () => {
  ...
  try {
    ...
    if (res.code === 200) { classStats.value = res.data.classes || []; summary.value = res.data.summary || {} }
  } catch { ElMessage.error('加载失败') }
  finally { examLoading.value = false }
}

const resetFilters = () => { filters.grade = ''; filters.classId = null; filters.taskId = null; loadExamData() }

// ── 作业完成 Tab ──────────────────────────────
const hwLoading = ref(false)
const hwList = ref([])
const hwFilters = reactive({ grade: '', classId: null, taskId: null })

const onHwGradeChange = () => { hwFilters.classId = null; loadHwData() }
const resetHwFilters = () => { hwFilters.grade = ''; hwFilters.classId = null; hwFilters.taskId = null; loadHwData() }

const loadHwData = async () => {
  hwLoading.value = true
  try {
    const params = {}
    if (hwFilters.grade) params.grade = hwFilters.grade
    if (hwFilters.classId) params.classId = hwFilters.classId
    if (hwFilters.taskId) params.taskId = hwFilters.taskId
    const res = await getScoreAnalysis(params)
    if (res.code === 200) {
      const data = res.data
      hwList.value = (data.classStats || []).map(c => ({
        ...c, title: data.taskTitle || '任务', taskId: data.taskId || c.taskId,
        subject: data.subject || c.subject || '-', teacherName: data.teacherName || '-',
...
const onTabChange = (tab) => {
  if (tab === 'exams' && classStats.value.length === 0) loadExamData()
  else if (tab === 'homework' && hwList.value.length === 0) loadHwData()
}
...
onMounted(async () => {
  ...
  loadExamData()
  try {
    const [gradeRes, clsRes, taskRes] = await Promise.all([getGrades(), getClassList(), listTasks({ size: 999 })])
    if (gradeRes.code === 200) gradeOptions.value = (gradeRes.data || []).map(g => g.gradeName)
    if (clsRes.code === 200) classOptions.value = (clsRes.data.records || []).map(c => ({ id: c.id, className: c.className, grade: c.grade || '' }))
...
.inspector-task-analysis { max-width: 1280px; margin: 0 auto; padding: var(--spacing-lg, 24px); }
...
.summary-row { display: flex; gap: 16px; flex-wrap: wrap; margin-bottom: 20px;
  .summary-item { padding: 14px 20px; background: var(--bg-card); border-radius: var(--radius-md); text-align: center; box-shadow: var(--shadow-sm); min-width: 90px;
    ...
    &.primary .s-val { color: var(--primary-color); } &.good .s-val { color: var(--success-color); } &.bad .s-val { color: var(--danger-color); }
  }
}
.section-title { font-size: var(--fs-lg); font-weight: 600; margin: 20px 0 12px; color: var(--text-primary); }
.class-cards { display: grid; grid-template-columns: repeat(auto-fill, minmax(220px, 1fr)); gap: 14px;
  .class-card { background: var(--bg-card); border-radius: var(--radius-lg); padding: 18px; box-shadow: var(--shadow-sm); transition: all 0.2s;
    ...
    .metric { text-align: center;
      ...
      &.primary .m-val { color: var(--primary-color); } &.good .m-val { color: var(--success-color); } &.bad .m-val { color: var(--danger-color); }
    }
    ...
    &:hover .cc-arrow { color: var(--el-color-primary-dark-2); }
  }
}
...
.class-detail { .detail-section { margin-top: 20px; h4 { font-size: var(--fs-md); margin: 0 0 10px; color: var(--text-primary); } } }
...
@media (max-width: 768px) {
  .inspector-task-analysis { padding: var(--spacing-md, 16px); }
  .filter-bar { flex-direction: column; align-items: stretch; :deep(.el-select) { width: 100%; } :deep(.el-button) { width: 100%; margin-left: 0; } }
