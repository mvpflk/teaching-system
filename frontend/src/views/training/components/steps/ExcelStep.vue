<template>
  <div class="step-excel">
    ...
    <div v-if="step.config?.checkpoints?.length" class="requirements">
      ...
      <ul>
        <li v-for="(cp, i) in step.config.checkpoints" :key="i">
          <el-tag size="small" :type="cpType(cp.type)">{{ cpTypeLabel(cp.type) }}</el-tag>
          {{ cp.desc }}
        </li>
      </ul>
    </div>
    ...
    >
      ...
      <div>将 Excel 文件拖到此处或点击上传</div>
      <template #tip>
        <div class="el-upload__tip">仅支持 .xlsx 格式（Excel 2007+）</div>
      </template>
    </el-upload>
    ...
    <div v-if="checkResult" class="check-result">
      <el-alert
        :title="`自动评分: ${checkResult.passedCount || 0}/${checkResult.totalCount || 0} 项通过 (${checkResult.score || 0}分)`"
      ...
      <div v-for="c in (checkResult.checkpoints || [])" :key="c.id" class="checkpoint-item">
        <span :style="{ color: c.passed ? 'var(--el-color-success, #67c23a)' : 'var(--el-color-danger, #f56c6c)' }">
          {{ c.passed ? '✅' : '❌' }}
        </span>
        <span class="cp-desc">{{ c.desc }}</span>
        <span v-if="!c.passed && c.detail" class="cp-detail">{{ c.detail }}</span>
        <span class="cp-score">{{ c.score || 0 }}分</span>
      </div>
    </div>
  </div>
</template>
...
const uploadUrl = computed(() => `/api/training/upload/excel/${props.taskId}`)
...
function cpType(type) {
  const map = { formula: 'success', value: 'warning', format: 'info', chart: 'info' }
  return map[type] || ''
}
function cpTypeLabel(type) {
  const map = { formula: '公式', value: '数值', format: '格式', chart: '图表' }
...
<style scoped>
.step-excel { display: flex; flex-direction: column; gap: 16px; }
...
.requirements ul { margin: 0; padding-left: 20px; max-height: 300px; overflow-y: auto; }
.requirements li { margin: 6px 0; font-size: var(--fs-sm); display: flex; gap: 8px; align-items: center; }
...
.cp-score { font-weight: 600; color: var(--el-color-primary); min-width: 36px; text-align: right; }
</style>
